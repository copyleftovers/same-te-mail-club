...
- **`z-index` without documentation.** The grain overlay is `z-index: 1`. Dropdowns are `z-index: 40`. Mobile menu overlay is `z-index: 40`. Modals are `z-index: 50`. Confetti burst overlay is `z-index: 60` (above modals; nothing sits above confetti during the delivery phase). Toast is in normal block flow (no z-index). Document any new layer.
