...
## Added 2026-07-11 (T-CAP capture-elevation close)

- **RESOLVED: cycle-viz 11-15-node cohort capture gap** (carried since 2026-06-25) — 12-node SQL-seeded cohort pass now captures it in all 4 scheme variants + section crop. The capture PROVED the collision defect (below).
- **NEW: app-defect catalog from rendered-verify** — `orchestration_log/history/2026-07-09/app-defect-catalog.md` (tracked). 7 actionable: A1 cycle-viz 12-node label collisions (3 zones, MAJOR, all modes/viewports); A2 404 page unstyled (MAJOR); A3 toast splits/obscures admin content (BLOCKER-rated); A4 error borders applied to ALL create-season fields incl. valid ones in dark error-state (MAJOR — aria-invalid routing suspect); A5 phone numbers styled accent-orange (MAJOR); A6 deactivated-row em-dash near-invisible in dark (MAJOR); A9 alert fill imperceptible (MINOR). Plus A10 design question: unlaunched season is invisible to participants (home.rs `launched_at IS NOT NULL`) — "announced, opens soon" state doesn't exist. A7 (advance disabled pre-generate) + A8 (onboarding lacks auth-card) resolved NOT-A-DEFECT with source citations. Awaiting user fault-intake prioritization.
- **Known intermittent tower_http 500** (Leptos SSR reactive-disposal, pre-existing) appeared once per capture run, zero test impact — unchanged, still tracked.
- **Minor deferred from unit-7 quality review:** MOBILE_VIEWPORT/DESKTOP_VIEWPORT/LAYOUT_REFLOW_MS constants duplicated between visual-audit.spec.ts and visual-audit-cohort.spec.ts (silent-drift risk; extract shared fixture on next touch). Residual narrow race in the H1 no-season guard (point-in-time isVisible after main-visible wait can false-negative mid-Suspense in contaminated full-suite runs — degrades to pre-fix behavior only).

## Updated 2026-07-11 (T-FIX close)
...
## Audited 2026-07-13 (deferred-items accuracy sweep)

8-agent forensic audit (~50 sub-claims, full document history) against primary sources at HEAD 876dd8d. Verdict: accurate document, small drift, zero fabrication. 4 corrections below; ~40 other claims independently CONFIRMED, no action needed. Full rollup: `orchestration_log/recon/2026-07-13/deferred-verify/SYNTHESIS.md`.

- **Correction to "T08 cycle-viz desktop scale-down" (2026-07-10 section):** the cited `viewBox 600×560` is stale, not false. Current `src/admin/page.rs` viewBox is `900×750` — widened by the T-FIX campaign (2026-07-11, "A1 cycle-viz label collisions") for the 12-node label-collision fix, already documented in this file's own 2026-07-11 sections. The 600×560 mechanism (viewBox matches aspect-ratio, named geometry constants) is still true; only the specific dimensions changed.
- **Correction to "auth /login SSR abort" resolution (2026-07-10 section):** the cited SHA `5da012b` is a 3-line comment-only commit, not the fix itself. The substantive integration is `ee816fd` (page-frame/auth-card wrap + OTP resend), `5cf9843` (fix the `>` misparse inside `disabled={...}`), and `b892ab0` (cooldown closure extraction) — all ancestors of `5da012b` in the same chain. Anyone tracing this fix should start at `ee816fd`.
- **Correction to "SMS predicate dedup" (2026-07-12 sustainability-close section, "ActiveSeasonRow + SMS predicate dedup"):** overstates scope. Only `send_season_open_sms` calls the shared `fetch_active_launched_season` predicate. The other 3 send functions (`send_assignment_sms`, `send_confirm_nudge_sms`, `send_receipt_nudge_sms`) each inline their own phase-scoped season-lookup query — **by explicit design**, per `src/admin/sms.rs:252,342,440`, each carrying the verbatim comment "Intentionally narrower than the canonical active-launched predicate." What is genuinely one-homed: the count↔send predicate *pairing* (each `count_*` helper's SQL body matches its corresponding send function's target query, both called from `state.rs::query_sms_counts`) — not a single shared season-lookup across all 4 send functions.
- **Resolution of the "MOBILE_VIEWPORT/DESKTOP_VIEWPORT/LAYOUT_REFLOW_MS duplicated" note (2026-07-11 T-CAP section, "Minor deferred from unit-7 quality review"):** RESOLVED, not still open. Current source shows both `end2end/tests/visual-audit.spec.ts` and `end2end/tests/visual-audit-cohort.spec.ts` import all three constants from the shared `end2end/tests/fixtures/capture-constants.ts` fixture (grep-confirmed, zero local redeclarations). Either a later commit fixed this silently or it was never duplicated in the shipped code — either way, the flagged risk does not exist in current source.
