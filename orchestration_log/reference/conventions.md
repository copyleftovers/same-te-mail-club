...
## Added 2026-07-13 (deferred-items doc discipline)

**`deferred_items.md` holds ONLY currently-open items — resolve means DELETE, not stamp.** The doc is a LIVE debt list, not an append-only changelog. Format: flat list of open items; no historical narrative, no "RESOLVED"/"CLOSED"/struck-through entries kept in the live file. The moment an item is confirmed fixed, DELETE it from the file entirely — in the SAME commit that would otherwise have marked it resolved. The historical fact that it was ever open and later fixed already has ONE durable home: `codebase_state.md`'s dated `## Changes` sections + the frozen `orchestration_log/history/*/session.md` narratives. Restating it in `deferred_items.md` is knowledge duplication (two homes drift) and dead weight that every future ARRIVE re-reads. User's words: "the moment we resolve something, we ought to strike it off the deferred items, erasing completely, not stamping the 'resolved' flag on the dead confusing weight."

**RE-VERIFY every still-listed item against CURRENT source before EVERY close (or any touch of the file).** Do not carry an item forward from memory or from the previous session's wording — grep the actual defect signature / read the current source and confirm it is still open. The doc rots in BOTH directions: "we're behind" (a fixed item still listed) is as real as "we're ahead," and the behind-direction is MORE dangerous — it wastes future effort re-investigating a solved problem. This is a recurring failure mode: an item silently fixed as a side effect of unrelated work re-lists unverified each session. Traced: 2026-07-13 accuracy audit found the viewport-constant test-fixture duplication (unit-7 deferral) had been silently fixed by an unrelated refactor 2+ sessions earlier, still re-listed as open, unverified (corrected in `b6d0a00`).

**Forbidden (deferred-items additions):**
| Pattern | Why | Traced to |
|---|---|---|
| Marking a deferred item RESOLVED/CLOSED and leaving it in `deferred_items.md` | Dead weight + next-session confusion; the file becomes an append-only changelog instead of a live debt list — that role belongs to `codebase_state.md` Changes sections + `history/`. Resolve = delete in the same commit. | 2026-07-13: user correction |
| Carrying a deferred item forward unverified against current source | The list rots "behind" (fixed items re-listed), wasting future effort re-investigating solved problems. Re-verify every item before close. | 2026-07-13: viewport-constants drift (fixed 2+ sessions prior, still listed), audit-corrected `b6d0a00` |
